...
class ControllerExtensionModuleFeatured extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['custom_image'])) {
			$data['custom_image'] = $this->request->post['custom_image'];
		}elseif (!empty($module_info)) {
			$data['custom_image'] = $module_info['custom_image'];
		} else {
			$data['custom_image'] = "";
		}
		...
		if (isset($this->request->post['custom_image']) && is_file(DIR_IMAGE . $this->request->post['custom_image'])) {
			$data['thumb'] = $this->model_tool_image->resize($this->request->post['custom_image'], 100, 100);
		} elseif ($data['custom_image'] && is_file(DIR_IMAGE . $data['custom_image'])) {
			$data['thumb'] = $this->model_tool_image->resize($module_info['custom_image'], 100, 100);
		} else {
...
// 
		...
		if (isset($this->request->post['bg_image']) && is_file(DIR_IMAGE . $this->request->post['bg_image'])) {
			$data['thumb2'] = $this->model_tool_image->resize($this->request->post['bg_image'], 100, 100);
		} elseif ($data['bg_image'] && is_file(DIR_IMAGE . $data['bg_image'])) {
			$data['thumb2'] = $this->model_tool_image->resize($module_info['bg_image'], 100, 100);
		} else {
