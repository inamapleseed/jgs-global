...
// debug($product_query);
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$checkout_tax = $product_query->row['checkout_tax'];
				// << Related Options / Связанные опции 
				...
				$product_special_query = $this->db->query("SELECT price, checkout_tax FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$cart['product_id'] . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00' OR date_start <= CURDATE()) AND (date_end = '0000-00-00' OR date_end >= CURDATE())) ORDER BY priority ASC, price ASC LIMIT 1");
				...
				// $checkout_tax = $product_special_query->row['checkout_tax'];
				...
				$product_data[] = array(
					...
					'checkout_tax'    => $checkout_tax,
					'total'           => ($price + $checkout_tax + $option_price) * $cart['quantity'],
    ...
    public function salesonecombo1(){
    	if(isset($this->session->data['slscmdprdts']) && !empty($this->session->data['slscmdprdts'])) {
        //$this->log->write("wholesaleconditionexists");$this->log->write(print_r($this->session->data['slscmdprdts'],true));
