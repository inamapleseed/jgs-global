...
class ModelCatalogManufacturer extends Model {
	...
	public function getProductRelated2($manufacturer_id) {
		...
		$query = $this->db->query("SELECT pd.name, mr.manufacturer_id, p.product_id, p.price, p.image FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "manufacturer_related mr ON (p.product_id = mr.mrelated_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer_to_store m2s ON (p.product_id = m2s.manufacturer_id) WHERE mr.manufacturer_id = '" . (int)$manufacturer_id . "'");
		// debug($query->rows);
		// foreach ($query->rows as $result) {
		// 	$mproduct_data[$result['mrelated_id']] = $this->getManufacturer($result['mrelated_id']);
		// }
		// debug($query->rows);
		return $query->rows;
	}
