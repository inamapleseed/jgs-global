...
class ModelCatalogProduct extends Model {
	...
	public function getProductMainCategories2($product_id, $level = 0){
		$query= $this->db->query('SELECT * FROM `' . DB_PREFIX . 'category_description` cd LEFT JOIN `' . DB_PREFIX . 'product_to_category` p2c ON (cd.category_id = p2c.category_id) WHERE p2c.product_id="' . (int)$product_id . '" ');

		return $query->rows;
	}

	public function getProductManufacturer($product_id, $level = 0){
		$query= $this->db->query('SELECT m.name FROM `' . DB_PREFIX . 'product` p LEFT JOIN `' . DB_PREFIX . 'manufacturer` m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id="' . (int)$product_id . '" ');
	...
	public function getProduct($product_id) {
		...
		(SELECT ps.pricewtax FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start <= CURDATE()) AND (ps.date_end = '0000-00-00' OR ps.date_end >= CURDATE())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, 
		(SELECT ps.tax_label FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start <= CURDATE()) AND (ps.date_end = '0000-00-00' OR ps.date_end >= CURDATE())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS tax_label, 
		(SELECT ps.checkout_tax FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start <= CURDATE()) AND (ps.date_end = '0000-00-00' OR ps.date_end >= CURDATE())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS checkout_tax, 
		...
		if ($query->num_rows) {
			...
			return array(
				...
				'tax_label'        => $query->row['tax_label'],
				'checkout_tax'     => $query->row['checkout_tax'],
