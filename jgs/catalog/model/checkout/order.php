	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', checkout_tax = '" . (float)$product['checkout_tax'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', sku='". $this->db->escape($sku) ."'");
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				foreach ($order_product_query->rows as $product) {
					...
					$data['products'][] = array(
						...
						'checkout_tax'    => $this->currency->format($product['checkout_tax'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
